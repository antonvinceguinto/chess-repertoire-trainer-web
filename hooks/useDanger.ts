...
export function useDanger(
  gaps: Gap[],
  enabled: boolean,
): { scores: Map<string, DangerScore>; done: number; total: number } {
  ...
  useEffect(() => {
    ...
    const engine = new StockfishEngine(
      (evaluation) => {
        if (cancelled) return;
        // Only act on the final snapshot of a search (running === false).
        if (evaluation.running || evaluation.lines.length === 0) return;
  ...
  return { scores, done: scores.size, total: gapTargets.length };
}
